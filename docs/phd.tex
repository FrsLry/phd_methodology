...
% The mind, the...
\hypertarget{title-phd-methodology-if-you-indicate-the-tile-itll-create-a-new-cover-page}{%
\chapter{title: ``PhD Methodology'' \# if you indicate the tile, it'll create a new cover page}\label{title-phd-methodology-if-you-indicate-the-tile-itll-create-a-new-cover-page}}

Placeholder

\hypertarget{this-line-is-to-use-when-you-want-several-outputs-specified-in-output.yml}{%
\section{This line is to use when you want several outputs (specified in output.yml)}\label{this-line-is-to-use-when-you-want-several-outputs-specified-in-output.yml}}

\hypertarget{the-default-class}{%
\section{The default class}\label{the-default-class}}

\hypertarget{to-colorize-the-biblio-ref-and-the-urls}{%
\section{to colorize the biblio ref and the URLs}\label{to-colorize-the-biblio-ref-and-the-urls}}
...
complexity and explicative power \autocite{houlahan_priority_2017,levins_strategy_1966}. Thus, using as few covariates as possible to
...
the Czech Republic, which is encouraging (see \href{https://github.com/FrsLry/IGA_figures/blob/main/pred_vs_obs.pdf}{Fig. 2}). An other advantage of the complex
nonparametric models is that you can represent the dependence between the outcome and a
predictor of interest called a marginal plot or partial plot. For instance, in \href{https://github.com/FrsLry/IGA_figures/blob/main/marginal_plot.pdf}{Fig.3}, I represented the influence of the
...
overfitting, \textbf{2)} add the adequate environmental parameters.
...
01/09/2021\strut
\end{minipage} & \begin{minipage}[t]{0.49\columnwidth}\raggedright
Building models for Czech Republic: machine learning models are powerful but need to be parametrized in order to increase predicting power and avoid overfitting. Moreover, I will build my models with significant databases and expect to run long time computing jobs.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.21\columnwidth}\raggedright
01/01/2022\strut
\end{minipage} & \begin{minipage}[t]{0.21\columnwidth}\raggedright
...
\item
  Better understand the link between spatial and temporal features (\emph{e.g.} latitude, longitude, area, elongation, time span) and biodiversity changes.
\item
